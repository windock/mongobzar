...
describe 'Embedded association' do
  ...
  let(:person_repository) do
    res = Repository::Repository.new('testing', 'people')
    address_assembler = Assembler::SimpleAssembler.new(->(dto) { Address.new(dto['street']) }, [:street])
    res.assembler = Assembler::EntityAssembler.new(PersonAssembler.new(address_assembler))
    res
  end
  let(:person) { Person.new }
  let(:matcher) { AddressMappingMatcher.new(people_collection) }
  ...
  describe 'one' do
    let(:work_address) { Address.new('Work street') }
  ...
  describe 'many' do
    let(:address) { Address.new('street') }
    let(:address2) { Address.new('street2') }
    let(:address3) { Address.new('street3') }
    let(:address4) { Address.new('street4') }
